 ...
 */
public class AlignToTag14FieldRelativePose3D extends Command {
    static {
        System.out.println("Loading: " + java.lang.invoke.MethodHandles.lookup().lookupClass().getCanonicalName());
    }

    private int tagIDDesired = 14; // FIXME need dynamic determination of the tag to use (and change name of class)
    ...
    public AlignToTag14FieldRelativePose3D(VisionContainer visionContainer, Drivetrain drivetrain) {

        this.visionContainer = visionContainer;
        this.drivetrain = drivetrain;
        addRequirements(drivetrain);

        // need the robot's position to score. Let's use the tag 14 + offsets for the target's pose3d
        // location of the robot to score relative to the tag in field coordinates
        var targetToTag = new Transform3d(1., 0.17, 0., new Rotation3d(0., 0., Math.PI)); // offset from AprilTag in field pose
